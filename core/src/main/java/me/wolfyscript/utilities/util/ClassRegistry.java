...
public interface ClassRegistry<V extends Keyed> extends Iterable<Class<? extends V>> {
    ...
    SimpleClassRegistry<Animator> PARTICLE_ANIMATORS = new WrapperClassRegistry<>(() -> WolfyUtilCore.getInstance().getRegistries().getParticleAnimators());
    SimpleClassRegistry<Timer> PARTICLE_TIMER = new WrapperClassRegistry<>(() -> WolfyUtilCore.getInstance().getRegistries().getParticleTimer());
    SimpleClassRegistry<Meta> NBT_CHECKS = new WrapperClassRegistry<>(() -> WolfyUtilCore.getInstance().getRegistries().getCustomItemNbtChecks());
