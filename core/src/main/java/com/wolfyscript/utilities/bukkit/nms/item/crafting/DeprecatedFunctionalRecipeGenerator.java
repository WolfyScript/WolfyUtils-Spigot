...
public class DeprecatedFunctionalRecipeGenerator implements FunctionalRecipeGenerator {
    ...
    private static abstract class ReflectionLookup {

        /* ******************
         * Minecraft classes
         * ******************/
        static final Class<?> CONTAINER_CLASS;
        static final Class<?> CRAFTING_CONTAINER_CLASS;
        static final Class<?> WORLD_CLASS;
        static final Class<?> ITEMSTACK_CLASS;
        static final Class<?> RECIPE_CLASS;
        static final Class<?> RECIPE_ITEMSTACK_CLASS;
        static final Class<?> RESOURCE_KEY_CLASS;
        static final Class<?> NON_NULL_LIST_CLASS;
        static final Class<?> RECIPE_MANAGER_CLASS;
        static final Class<?> MINECRAFT_SERVER_CLASS;
    ...
    public DeprecatedFunctionalRecipeGenerator(WolfyUtilCore core) {
        this.core = core;
        ...
        this.mappings = Map.ofEntries(
                ...
                // NMS
                entry("NMS_ItemStack", ReflectionLookup.ITEMSTACK_CLASS.getName()),
                entry("NMS_ItemStack_Empty", ReflectionLookup.ITEMSTACK_EMPTY_CONST.getName()),
                entry("Level", ReflectionLookup.WORLD_CLASS.getName()),
                entry("ItemStack", ReflectionLookup.ITEMSTACK_CLASS.getName()),
                entry("Container", ReflectionLookup.CONTAINER_CLASS.getName()),
                entry("CraftingContainer", ReflectionLookup.CRAFTING_CONTAINER_CLASS.getName()),
                entry("NonNullList", ReflectionLookup.NON_NULL_LIST_CLASS.getName()),
                entry("NonNullList_Create", ReflectionLookup.NONNULLLIST_WITH_SIZE_METHOD.getName()),
                entry("RecipeMatchesMethod", ReflectionLookup.RECIPE_MATCHES_METHOD.getName()),
                entry("RecipeAssembleMethod", ReflectionLookup.RECIPE_ASSEMBLE_METHOD.getName()),
                entry("RemainingItemsMethod", ReflectionLookup.RECIPE_GET_REMAINING_ITEMS_METHOD.getName()),
                entry("Ingredient_Empty", ReflectionLookup.RECIPE_ITEMSTACK_EMPTY_CONST.getName()),
                entry("Ingredient", ReflectionLookup.RECIPE_ITEMSTACK_CLASS.getName()),
                entry("Ingredient_Choices",
                        // Minecraft 1.17+
                        Arrays.stream(ReflectionLookup.RECIPE_ITEMSTACK_CLASS.getMethods()).filter(field -> field.getReturnType().equals(ReflectionLookup.ITEMSTACK_CLASS.arrayType())).findFirst().map(method -> method.getName() + "()")
                                // Minecraft 1.16
                                .or(() -> Arrays.stream(ReflectionLookup.RECIPE_ITEMSTACK_CLASS.getFields()).filter(field -> field.getType().equals(ReflectionLookup.ITEMSTACK_CLASS.arrayType())).map(Field::getName).findFirst())
                                // Fallback
                                .orElse("choices")
                ),
                //// 1.19.4
                entry("1_19_4_AssembleRegistry", ServerVersion.isAfterOrEq(MinecraftVersion.of(1, 19, 4)) ? (", " + ReflectionLookup.CUSTOM_REGISTRY_CLASS.getName() + " registry") : ""),
                ...
                entry("CraftItemStack", ReflectionLookup.CRAFT_ITEMSTACK_CLASS.getName()),
                entry("CraftInventory", ReflectionLookup.CRAFT_INVENTORY_CLASS.getName()),
                entry("CraftInventoryCrafting", ReflectionLookup.CRAFT_INVENTORY_CRAFTING_CLASS.getName()),
    ...
    public void generateRecipeClasses() {
        if (!ReflectionLookup.GENERATED_RECIPES.isEmpty()) return;
        core.getLogger().info("Generate Functional Recipes");
        try {
            ...
            if (!(core instanceof WolfyCorePaper)) { // Papers plugin Classloaders (The toString() method to be exact) cause a StackOverflow in some circumstances!!!
                ...
                logger.fine("Minecraft Classloader : " + ReflectionLookup.MINECRAFT_SERVER_CLASS.getClassLoader());
            }
    ...
    public Class<?> getFunctionalRecipeClass(FunctionalRecipeType type) {
        return ReflectionLookup.GENERATED_RECIPES.get(type);
    }
    ...
    public boolean addRecipeToRecipeManager(FunctionalRecipe<?> recipe) {
        try {
            ReflectionLookup.RECIPE_MANAGER_ADD_RECIPE_METHOD.invoke(ReflectionLookup.MINECRAFT_SERVER, recipe);
            return true;
        } catch (IllegalAccessException | InvocationTargetException e) {
            return false;
        }
    ...
    private Class<?> inject(ClassPool classPool, Class<?> originalClass) throws NotFoundException, CannotCompileException, IOException {
        ...
        for (Constructor<?> constructor : originalClass.getConstructors()) {
            ...
            for (int i = 0; i < parameters.length; i++) {
                ...
                if (parameters[i].equals(ReflectionLookup.RESOURCE_KEY_CLASS)) {
                    // new ResourceLocation(var0.getNamespace(), var0.getKey())
                    bodyBuilder.append("new ").append(ReflectionLookup.RESOURCE_KEY_CLASS.getName()).append("(");
                ...
                } else if (parameters[i].equals(ReflectionLookup.RECIPE_ITEMSTACK_CLASS)) {
                ...
                } else if (parameters[i].equals(ReflectionLookup.ITEMSTACK_CLASS)) {
                    // Use Bukkit ItemStack in the Constructor and convert to NMS
                    bodyBuilder.append(ReflectionLookup.CRAFT_ITEMSTACK_CLASS.getName()).append(".").append(ReflectionLookup.CRAFT_ITEMSTACK_TO_NMS.getName()).append("(").append(name).append(")");
                    // ItemStack var<i>
                    signatureBuilder.append(", ");
                    signatureBuilder.append(ItemStack.class.getName()).append(' ').append(name);
                } else if (parameters[i].equals(ReflectionLookup.NON_NULL_LIST_CLASS)) {
                ...
                } else if (ServerVersion.isAfterOrEq(MinecraftVersion.of(1, 19, 3)) && parameters[i].equals(ReflectionLookup.COOKING_BOOK_CATEGORY)) {
                    // Use String in the constructor and find the category for it.
                    bodyBuilder.append("(").append(ReflectionLookup.COOKING_BOOK_CATEGORY.getName()).append(")").append(ReflectionLookup.COOKING_BOOK_CATEGORY.getName()).append(".").append(ReflectionLookup.COOKING_BOOK_CATEGORY_CODEC.getName()).append(".a(").append(name).append(")");
                    // String var<i>
                    signatureBuilder.append(", ");
                    signatureBuilder.append("String ").append(name);
                } else if (ServerVersion.isAfterOrEq(MinecraftVersion.of(1, 19, 3)) && parameters[i].equals(ReflectionLookup.CRAFTING_BOOK_CATEGORY)) {
                    // Use String in the constructor and find the category for it.
                    bodyBuilder.append("(").append(ReflectionLookup.CRAFTING_BOOK_CATEGORY.getName()).append(")").append(ReflectionLookup.CRAFTING_BOOK_CATEGORY.getName()).append(".").append(ReflectionLookup.CRAFTING_BOOK_CATEGORY_CODEC.getName()).append(".a(").append(name).append(")");
                    // String var<i>
                    signatureBuilder.append(", ");
                    signatureBuilder.append("String ").append(name);
                } else {
        ...
        SignatureAttribute.ClassSignature classSignature = new SignatureAttribute.ClassSignature(null, null,
                ...
                new SignatureAttribute.ClassType[]{
                        new SignatureAttribute.ClassType(Function.class.getName(), new SignatureAttribute.TypeArgument[]{
                                ...
                                new SignatureAttribute.TypeArgument(new SignatureAttribute.ClassType(ReflectionLookup.ITEMSTACK_CLASS.getName()))
                        })
                }
        );
    ...
    private void generateConversionUtils(ClassPool classPool) throws CannotCompileException, IOException {
        ...
        Field emptyIngredientField = Arrays.stream(ReflectionLookup.RECIPE_ITEMSTACK_CLASS.getDeclaredFields()).filter(field -> field.getType().equals(ReflectionLookup.RECIPE_ITEMSTACK_CLASS)).findFirst().orElse(null);
